...
## Usage Guide
see test/test_forkzstream_ws.py
### Module Loading

To use the fork_zstream module, you first need to load it into FreeSWITCH:
...
The module provides a FreeSWITCH application called `forkzstream` with the following commands:
...
- **params**: WebSocket service parameters in JSON format, including:
  - `url`: WebSocket server URL
  - `text`: Text to be synthesized (optional)
...
- **reqid**: The unique identifier of the request to stop

### Example Usage

#### Using WebSocket TTS Service

```
forkzstream start 12345 websocket {"url":"ws://localhost:8080/tts"}
```

### Event Handling

The module generates custom events for various operations. You can listen for these events in your FreeSWITCH applications:

- `mod_forkzstream::connect`: Connection established event
- `mod_forkzstream::disconnect`: Connection terminated event
- `mod_forkzstream::error`: Error occurred event
- `mod_forkzstream::json`: JSON message received event
- `mod_forkzstream::play`: Audio playback event

## 1. Project Overview

fork_zstream is a real-time audio stream bidirectional forwarding module based on FreeSWITCH, focused on implementing bidirectional audio data transmission between FreeSWITCH and external WebSocket services. The module captures FreeSWITCH's audio stream through media bug technology, forwards downstream data (ASR stream) to WebSocket services, while receiving upstream data (TTS stream) from WebSocket services and playing it in real-time, achieving full-duplex forwarding processing of FreeSWITCH data streams.

### 1.1 Core Features

- **Bidirectional Audio Stream Forwarding**: Implement bidirectional audio data transmission between FreeSWITCH and WebSocket services
...
- **Event Callback Mechanism**: Implement interaction between the module and FreeSWITCH through event callbacks
...
The fork_zstream module acts as a loadable module for FreeSWITCH, integrating with the core system through FreeSWITCH's API, implementing bidirectional communication with WebSocket services. Its overall architecture is as follows:
...
Bidirectional data forwarding is the core function of the fork_zstream module, implementing bidirectional audio data transmission between FreeSWITCH and WebSocket services through media bug.
...
This callback function is the core of the module, responsible for handling media events and implementing bidirectional data forwarding:
...
Send text to the WebSocket service for TTS synthesis.
...
The fork_zstream module provides powerful real-time audio stream bidirectional forwarding functionality for FreeSWITCH, implementing full-duplex forwarding processing of FreeSWITCH data streams through WebSocket protocol communication with external services.

### 7.1 Technical Value

- **Implemented bidirectional real-time audio data transmission between FreeSWITCH and external services**: Provides flexible audio processing capabilities for voice applications
- **Based on WebSocket protocol**: Implemented efficient communication with external services
